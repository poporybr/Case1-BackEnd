    ...
    static inserir(conteudo){
        ...
        return new Promise((resolve, reject) => {
            db.run(query, [conteudo.titulo, conteudo.descricao, conteudo.porcentagem], function (err){
                if (err){
                    reject({
                        mensagem: 'Erro ao inserir o conteudo',
                        erro: err
                    })
                }
    ...
    static deletar(id) {
        ...
        return new Promise((resolve, reject) =>{
            db.run(query, [id], (err) => {
                ...
                resolve({mensagem: 'Conteudo deletado com sucesso'})
            });
        });
    }
    ...
    static atualizar(id, conteudo){
        ...
        return new Promise((resolve, reject) => {
            db.run(query, [conteudo.titulo, conteudo.descricao, conteudo.porcentagem, id], (err) => {
                if (err){
                    reject({
                        mensagem: 'Erro ao atulizar o conteudo',
                        erro: err
                    })
                }

                resolve ({ mensagem: 'Conteudo atualizado com sucesso'})
            });
        });
    }
}
